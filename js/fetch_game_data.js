...
async function fetchAllWeeks() {
  ...
  for (let week = 1; week <= 18; week++) {
    try {
      ...
      const response = await axios.get(`${baseUrl}_/week/${week}/year/2025/seasontype/2`);
      ...
      $('.Card.gameModules').each((index, element) => {
        ...
        $(element).find('.Scoreboard').each((i, gameElement) => {
          ...
          // Game Time
          let gameTime = $(gameElement).find("div.ScoreCell__Time, div.ScoreboardScoreCell__Time").first().text().trim() || null;
  ...
  fs.writeFileSync('data/game_data/game_data.json', JSON.stringify(allWeeks, null, 2));
  console.log('All weeks fetched and stored in game_data.json');
}
